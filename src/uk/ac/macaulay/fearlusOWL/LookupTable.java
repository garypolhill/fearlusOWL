/*
 * uk.ac.macaulay.fearlusOWL: LookupTable.java
 * 
...
public class LookupTable<I, O> {
   ...
   */
  private LookupTable(String[] labels, final O outcome, Iterator<I> input) {
    this(labels);
  ...
  public void add(final O outcome, Iterator<I> input) {
    ...
    if(!input.hasNext()) {
      ...
      if(next.containsKey(this_input)) {
        ...
        next.put(this_input, new LookupTable<I, O>(labels, outcome, input));
      }
    }
  }
  ...
  public O lookup(Iterator<I> input) {
    ...
    if(!input.hasNext()) {
      ...
      if(!last.containsKey(this_input)) return null;
      return last.get(this_input);
    }
    else {
      if(next == null) return null;
      if(!next.containsKey(this_input)) return null;
  ...
  private void print(String before) {
    if(last != null) {
      for(I key: last.keySet()) {
        System.out.println(before + key + " = " + last.get(key));
      }
    }
    if(next != null) {
      for(I key: next.keySet()) {
        next.get(key).print(before + key + ", ");
      }
    }
  }
}
